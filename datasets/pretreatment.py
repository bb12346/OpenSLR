...
parser.add_argument('--input_path', default='/root/SSL/data/data_sorted/', type=str,
                    help='Root path of raw dataset.')
parser.add_argument('--output_path', default='/root/SSL/data/data_sorted_RGB_npy/', type=str,
...
parser.add_argument('--log', default=False, type=boolean_string,
                    help='If set as True, all logs will be saved. '
                         ...
                         'Default: False')
parser.add_argument('--worker_num', default=18, type=int,
...
def cut_pickle(seq_info, pid):
    ...
    for _frame_name in frame_list:
        ...
        # print('-img2-',img.shape,type(img))
        img = img.transpose(2,0,1)
        if np.size(img, 1)!=720 or np.size(img, 2)!=1280:
            print('-img-',img.shape,frame_path)
    ...
    if count_frame < 1:
        message = 'seq:%s, less than 1 valid data.' % (
            '-'.join(seq_info))
        warn(message)
        log_print(pid, WARNING, message)
