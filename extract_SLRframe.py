...
def generate_frame(txt_path, video_path, save_path):
    total_count = 0 
    txt_list  = sorted(os.listdir(txt_path))
    print('len(txt_list) =',len(txt_list))
    for folder_name in txt_list:
        ...
        for i in range(len(list_GTtxt)):
            ...
            if not isExists:
                ...
                for i in range(len(currenttime)):
                    ...
                    frame_id ='%03d' % (i+1)
                    cv2.imwrite(save_path_each+frame_id+'.png', currentframe[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--txt_path", default='/data/SSL/TRAIN/MSSL_TRAIN_SET_GT_TXT/', type=str, help="GT txt."
    )
    parser.add_argument(
        "--video_path", default='/data/SSL/TRAIN/MSSL_TRAIN_SET_VIDEOS_ELAN/', type=str, help="video_path."
    )
    parser.add_argument(
        "--save_path", default='/root/SSL/data_sorted/', type=str, help="save_path."
    )
    args = parser.parse_args()

    generate_frame(args.txt_path, args.video_path, args.save_path)