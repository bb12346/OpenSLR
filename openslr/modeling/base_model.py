...
class BaseModel(MetaModel, nn.Module):
    ...
    def _load_ckpt(self, save_name):
        ...
        for k in checkpoint.keys():
            if not k.startswith('mlp.logits') and not k.startswith('i3d.logits.conv3d') and not k.startswith('mlp.adaptor.2'):
    ...
    def inputs_pretreament(self, inputs):
        ...
        # print('--------1-------')
        seqs = [np2var(np.array([trf(fra) for fra in seq]), requires_grad=requires_grad).float()
        ...
        #     # s2 = []
        #     HW = trf(seq[0])
        #     # print('-len(seq)-',len(seq),len(seq[0]), len(seq[0][0]), np.size(HW,-2), np.size(HW,-1))
        #     # result = np.zeros(len(seq) * len(seq[0]) * len(seq[0][0]) * np.size(HW,-2) * np.size(HW,-1) ).reshape(len(seq), len(seq[0]), len(seq[0][0]), np.size(HW,-2), np.size(HW,-1))
        #     # print('-result-',result.shape)
        #     # for fra in seq:
        #     #     s2.append(trf(fra))
        #     #     print(trf(fra).shape)
        #     result = []
        #     for kk in range(len(seq)):
        #         # result[kk,] = trf(seq[kk])
        #         result.append(torch.from_numpy(trf(seq[kk])))
        #     result = torch.stack(result).numpy()
        # s1.append(np2var(result, requires_grad=requires_grad).float())
        # seqs = s1
        # print('--------2-------')
        typs = typs_batch
        vies = vies_batch
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            # print('-len(inputs)-',len(inputs),type(inputs))
            # print('-len(inputs[0])-',len(inputs[0]),type(inputs[0]))
            # print('-len(inputs[0][0])-',len(inputs[0][0]),type(inputs[0][0]))
