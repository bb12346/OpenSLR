...
class CollateFn(object):
    ...
    def __call__(self, batch):
        ...
        # for seqs in seqs_batch:
            ...
            # print(type(seqs[0]),len(seqs[0]),seqs[0].shape)
            # sf = sample_frames(seqs)
            # print(type(sf),len(sf))
            # print(type(sf[0]),len(sf[0]))
        ...
        #             # print(i,j)
        #             # print(type(fras_batch[i][j]),len(fras_batch[i][j]))
        #             # print(type(fras_batch[i][j][0]),len(fras_batch[i][j][0]),fras_batch[i][j][0].shape)
        #             # result = np.zeros(len(fras_batch[i][j]) * len(fras_batch[i][j][0]) * len(fras_batch[i][j][0][0]) * len(fras_batch[i][j][0][0][0]) ).reshape(len(fras_batch[i][j]), len(fras_batch[i][j][0]), len(fras_batch[i][j][0][0]), len(fras_batch[i][j][0][0][0]))
        #             # for k in range(len(fras_batch[i][j])):
        #             #     result[k,] = fras_batch[i][j][k]

        #             result = []
        #             for k in range(len(fras_batch[i][j])):
        #                 result.append(torch.from_numpy(fras_batch[i][j][k]))
        #             result = torch.stack(result).numpy()
        #             fras_batch_temp2.append(result)
        #             # print('-result-',i,result.shape)
        #         fras_batch_temp1.append(fras_batch_temp2)
        #     # print('-len(fras_batch_temp)-',len(fras_batch_temp))
        #     # print('-------------end---------------')
        #     fras_batch = fras_batch_temp1
        # # if self.sampler == "fixed":
        # #     # fras_batch = [[np.asarray(fras_batch[i][j]) for i in range(batch_size)]
        # #     #               for j in range(feature_num)]  # [f, b]
        # #     fras_batch = [[np.array(fras_batch[i][j]) for i in range(batch_size)]
        # #                   for j in range(feature_num)]  # [f, b]
            # print('-------------end---------------')
        else:
